...
VS_OUTPUT VSMain(float4 Pos: POSITION, float3 N:NORMAL,float3 T:TANGENT,float3 B:BINORMAL,float2 UV: TEXCOORD0,float2 UV1: TEXCOORD1)
{
   ...
   Out.UV7 = UV*2+float2(0,Time/2);//float2(Out.Pos.x/120,Out.Pos.z/120);
...
float4 brightPS(VS_OUTPUT v):COLOR{
   ...
   v.UV+=float2(dot(v.LocV,v.T),dot(v.LocV,v.B))*tex.b/32;//*(cos(Time)+1);
   tex=tex2D(Texture,v.UV);

   //float4 tex2=tex2D(Texture,v.UV+v.LD/512.0);
   float dT=(tex2D(Texture,v.UV+float2(0,1/256.0)).b-tex2D(Texture,v.UV+float2(0,-1/256.0)).b)*8;
   float dB=(tex2D(Texture,v.UV+float2(1/256.0,0)).b-tex2D(Texture,v.UV+float2(1/-256.0,0)).b)*8;
   ...
   float4 t3=tex2D(Brighter,v.UV7);
   t3.xyz*=1-tex.w;
   ...
   c1*=tex*D*TFactor*3;
   c1.w=TFactor.w;
   c1.xyz=lerp(c1.xyz,fog.xyz,fog.w)+tex*t3*6;
   return c1;
}
