...
float4 brightPS(VS_OUTPUT v):COLOR{
   ...
   float dT=(tex2D(Texture,v.UV+float2(0,1/256.0)).g-tex2D(Texture,v.UV+float2(0,-1/256.0)).g)*2;
   float dB=(tex2D(Texture,v.UV+float2(1/256.0,0)).g-tex2D(Texture,v.UV+float2(1/-256.0,0)).g)*2;
   ...
   float3 D=(LightAmbient-clamp(LightDiffuse*dot(nn,LDir),-1,0));
   float4 fog=tex2D(FogOfWar,v.UV3);
   float4 c1=1;//lerp(float4(2,2,2,0),v.D,tex.w);
   c1*=tex*D*TFactor*3;
   c1.w=TFactor.w;
   c1.xyz=lerp(c1.xyz,fog.xyz,fog.w)+S*S*S*S*tex.g/2;
   return c1;
}
