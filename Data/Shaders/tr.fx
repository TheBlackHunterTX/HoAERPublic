...
float4 brightPS(VS_OUTPUT v):COLOR{
   ...
   float dT=(tex2D(Texture,v.UV+float2(0,1/2048.0)).r-tex2D(Texture,v.UV+float2(0,-1/2048.0)).r)*8;
   float dB=(tex2D(Texture,v.UV+float2(1/512.0,0)).r-tex2D(Texture,v.UV+float2(1/-512.0,0)).r)*8;
   ...
   c1*=tex*D*TFactor*3*float4(1.4,1.3,1,1);
   c1.w=TFactor.w*v.D.w;
   c1.xyz=lerp(c1.xyz,fog.xyz,fog.w)+S*S*S*S/23;
   return c1;
}
