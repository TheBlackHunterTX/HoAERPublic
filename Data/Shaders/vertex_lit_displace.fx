...
float4 brightPS(VS_OUTPUT v):COLOR{
   ...
   float4 Bump0=tex2D(Texture2,v.UV);
   v.UV+=float2(dot(v.B,VDir),dot(v.T,VDir))*(Bump0.b-0.5)/30;
   float4 Bump=tex2D(Texture2,v.UV);
   float4 tex=tex2D(Texture,v.UV);
   
   float dT=(Bump.a-0.5)*4;
   float dB=(Bump.r-0.5)*4;
   ...
   c1*=tex*D*TFactor*3;
   c1.w=TFactor.w;
   c1.xyz=lerp(c1.xyz,fog.xyz,fog.w)+pow(S,8)*Bump.g;
   return c1;
}
