...
float4 brightPS(VS_OUTPUT v):COLOR{
   ...
   float dT=(tex2D(Texture,v.UV+float2(0,1/256.0)).w-tex2D(Texture,v.UV+float2(0,-1/256.0)).w)*2;
   float dB=(tex2D(Texture,v.UV+float2(1/256.0,0)).w-tex2D(Texture,v.UV+float2(1/-256.0,0)).w)*2;
   ...
   c1*=tex*D*TFactor*3;
   c1.w=TFactor.w;
   c1.xyz=lerp(c1.xyz,fog.xyz,fog.w)+S*S*S*S/3;
   return c1;
}
