...
VS_OUTPUT VSMain(float4 Pos: POSITION, float3 N:NORMAL,float3 T:TANGENT,float3 B:BINORMAL,float2 UV: TEXCOORD0,float2 UV1: TEXCOORD1)
{
   ...
   Out.UV7 = UV*2+float2(Time*1,0);//float2(Out.Pos.x/120,Out.Pos.z/120);
...
float4 brightPS(VS_OUTPUT v):COLOR{
   ...
   float dT=(tex2D(Texture,v.UV+float2(0,1/256.0)).r-tex2D(Texture,v.UV+float2(0,-1/256.0)).r)*2;
   float dB=(tex2D(Texture,v.UV+float2(1/256.0,0)).r-tex2D(Texture,v.UV+float2(1/-256.0,0)).r)*2;
   ...
   c1*=tex*D*TFactor*2;
   c1.w=TFactor.w;
   c1.xyz=lerp(c1.xyz,fog.xyz,fog.w)+S*S*S*S*tex.a+tex*t3*8;
   return c1;
}
