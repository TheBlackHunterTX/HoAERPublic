...
VS_OUTPUT VSMain(float4 Pos: POSITION, float4 D:COLOR0,float4 S:COLOR1, float4 C:COLOR2,float2 UV: TEXCOORD0)
{
   ...
   float3 T=D.xyz*2-1;
   float3 B=S.xyz*2-1;
   float3 N=normalize(cross(B,T)*(S.w*2-1));
...
float4 brightPS(VS_OUTPUT v):COLOR{
   ...
   float dT=(tex2D(Texture,v.UV+float2(0,1/256.0)).r-tex2D(Texture,v.UV+float2(0,-1/256.0)).r)*0.92;
   float dB=(tex2D(Texture,v.UV+float2(1/256.0,0)).r-tex2D(Texture,v.UV+float2(1/-256.0,0)).r)*0.92;
   ...
   c1*=tex*D*TFactor*3;
   c1.w=TFactor.w;
   c1.xyz=lerp(c1.xyz,fog.xyz,fog.w)+S*S*S*S*S*S*S*clamp(tex.r-tex.b,0,1);
   return c1;
}
