...
VS_OUTPUT VSMain(float4 Pos: POSITION, float4 D:COLOR0,float4 S:COLOR1, float4 C:COLOR2,float2 UV: TEXCOORD0)
{
   ...
   Out.B = normalize((cross(Out.N,Out.T)*(S.w*2-1)));
   //Out.Pos.xyz += cos(Time*50)*Out.T/30;
...
float4 brightPS_nmap(VS_OUTPUT v):COLOR{
   ...
   //use this define to test light
   float4 LDir=float4(cos(Time*2),sin(Time*2),cos(Time*1.4),0);
   LDir=float4(0,cos(Time),sin(Time),0);
#endif //RANDOMLIGHT   
   ...
   float4 tex=0.5;//tex2D(Texture,v.UV*3);
   float4 bump=tex2D(BumpSpec,v.UV);
   bump.xyz=0.5+normalize(2*bump.xyz-1)/2;
   LDir=float4(dot(v.B,LDir),dot(v.T,LDir),dot(v.N,LDir),0);
   float3 refl=0;//VDir-2*nn*dot(VDir,nn);
   float  S=max(0,dot(refl,LDir));

   float D=(LightAmbient-clamp(LightDiffuse*dot(bump.xyz*2-1,LDir),-1,0));
   float4 fog=tex2D(FogOfWar,v.UV3);
   float4 c1=tex*D*TFactor*3/3;//+S*S*S*clamp(tex.b+tex.g-tex.r,0,1)/2;
   c1.w=TFactor.w;
   //c1.xyz=0.5-v.N/2;
   
   //c1.xyz=0.5+v.N/2;//lerp(c1.xyz,fog.xyz,fog.w)+pow(S,8)*(tex.w+0.2)/30;
   c1.xyz=lerp(c1.xyz,fog.xyz,fog.w)+pow(S,16)*tex.g/1;
    
   return c1;
}
float4 brightPS_nmap2(VS_OUTPUT v):COLOR{

#ifdef RANDOMLIGHT
   //use this define to test light
   float4 LDir=float4(cos(Time*2),sin(Time*2),cos(Time*1.4),0);
   LDir=float4(0,cos(Time/4),sin(Time/4),0);
#endif //RANDOMLIGHT   
   ...
   //v.B=normalize(cross(v.N,v.T));//normalize(v.T);

   //float3 DN=(v.N*(tex.z-0.5) - v.T*(tex.y-0.5) - v.B*(tex.x-0.5))*2;
   //LDir=float4(dot(v.B,LDir),dot(v.T,LDir),dot(v.N,LDir),0);

   float3 DN=(v.N*(bump.z-0.5) + v.B*(bump.y-0.5) + v.T*(bump.x-0.5))*2;
   float3 nn=normalize(DN);
...
float4 PS_bumpspec(VS_OUTPUT v):COLOR{
   ...
   float4 tex=tex2D(Texture,v.UV);
   float4 Spec=tex2D(BumpSpec,v.UV);
   float  dT=(tex2D(BumpSpec,v.UV+float2(0,1/TEXSIZE)).r-tex2D(BumpSpec,v.UV+float2(0,-1/TEXSIZE)).r)*BUMPSTRENGTH;
   float  dB=(tex2D(BumpSpec,v.UV+float2(1/TEXSIZE,0)).r-tex2D(Texture,v.UV+float2(1/-TEXSIZE,0)).r)*BUMPSTRENGTH;
   float3 DN=v.N HSIGN v.T*dT HSIGN v.B*dB;
   float3 nn=normalize(DN);
   ...
   c1.xyz=lerp(c1.xyz,fog.xyz,fog.w)+pow(S,8)*Spec.w;
   return c1;
}
//------------------- shadows skinning ----------------------//
